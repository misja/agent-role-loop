...
De rolprompts en contracten in `core/` zijn Engelstalig (ze presteren consistenter in het Engels en zijn internationaal herbruikbaar); de lessen en oefeningen zijn Nederlandstalig. Deze lijst overbrugt de twee talen. De Engelse term is leidend in de artefacten; gebruik in je verslagen de Nederlandse. Waar het lesmateriaal naar een specifiek bestand in `core/` verwijst, is dat een klikbare link naar de [referentiesectie](referentie/index.md) op deze site: de Engelse bronbestanden uit de repository, ongewijzigd overgenomen; een verwijzing naar een map (zoals `core/`) blijft als code staan.
